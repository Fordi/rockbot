...
namespace format_v_2_1_1 {
    ...
    void file_io::read_all_stages(format_v_2_1_1::file_stages& stages_data_out) const
    {
        ...
        if (fread(&stages_data_out, sizeof(format_v_2_1_1::file_stages), 1, fp) == 0) {
            std::cout << "ERROR::file_io::read_all_stages - Error reading struct data from stage file: '" << filename << "'" << std::endl;
            std::fflush(stdout);
            exit(-1);
        }
        fclose(fp);
...
namespace format_v_2_1_2 {
    ...
    void file_io::read_all_stages(format_v_2_1_2::file_stages& stages_data_out)
    {
        ...
        if (fread(&stages_data_out, sizeof(format_v_2_1_2::file_stages), 1, fp) == 0) {
    ...
    void file_io::read_stage(format_v_2_1_2::file_stage &stages_data_out, short stage_n)
    {
        ...
        if (fread(&stages_data_out, sizeof(struct format_v_2_1_2::file_stage), 1, fp) == 0) {
            printf(">>file_io::read_game - Error reading struct data from stage file.\n");
            std::fflush(stdout);
            exit(-1);
        }
...
// -------------------------------------- NPCS -------------------------------------- //
        ...
        if (fread(&data_out.game_npcs, sizeof(file_npc)*FS_GAME_MAX_NPCS, 1, fp)  == 0) {
            std::cout << ">>file_io::read_game - Error reading data from game file '" << filename << "'." << std::endl;
            exit(-1);
        }
        fclose(fp);
...
// -------------------------------------- OBJECTS -------------------------------------- //
        ...
        if (fread(&data_out.objects, sizeof(file_object)*FS_GAME_MAX_NPCS, 1, fp)  == 0) {
            std::cout << ">>file_io::read_game - Error reading data from game file '" << filename << "'." << std::endl;
            exit(-1);
        }
        fclose(fp);
...
// -------------------------------------- PROJECTILES -------------------------------------- //
        ...
        if (fread(&data_out.projectiles, sizeof(file_projectile)*FS_MAX_PROJECTILES, 1, fp)  == 0) {
            std::cout << ">>file_io::read_game - Error reading data from game file '" << filename << "'." << std::endl;
            exit(-1);
        }
        fclose(fp);
...
// -------------------------------------- WEAPONS -------------------------------------- //
        ...
        if (fread(&data_out.weapons, sizeof(file_weapon)*FS_MAX_WEAPONS, 1, fp) == 0) {
            std::cout << ">>file_io::read_game - Error reading data from game file '" << filename << "'." << std::endl;
            exit(-1);
        }
        fclose(fp);
...
// -------------------------------------- PLAYERS -------------------------------------- //
        ...
        if (fread(&data_out.players, sizeof(file_player)*FS_MAX_PLAYERS, 1, fp) == 0) {
            std::cout << ">>file_io::read_game - Error reading data from game file '" << filename << "'." << std::endl;
            exit(-1);
        }
        fclose(fp);
...
// -------------------------------------- ARTIFICIAL INTELIGENCE -------------------------------------- //
        ...
        if (fread(&data_out.ai_types, sizeof(file_artificial_inteligence)*FS_MAX_AI_TYPES, 1, fp) == 0) {
            std::cout << ">>file_io::read_game - Error reading data from game file '" << filename << "'." << std::endl;
            exit(-1);
        }
        fclose(fp);
...
// -------------------------------------- TROPHIES -------------------------------------- //
        ...
        if (fread(&data_out.trophies, sizeof(st_file_trophy)*TROPHIES_MAX, 1, fp) == 0) {
            std::cout << ">>file_io::read_game - Error reading data from game file '" << filename << "'." << std::endl;
            exit(-1);
        }
        fclose(fp);
...
// -------------------------------------- ARMOR PIECES -------------------------------------- //
        ...
        if (fread(&data_out.armor_pieces, sizeof(st_armor_piece)*FS_PLAYER_ARMOR_PIECES_MAX, 1, fp) == 0) {
            std::cout << ">>file_io::read_game - Error reading data from game file '" << filename << "'." << std::endl;
            exit(-1);
        }
        fclose(fp);
...
// -------------------------------------- ANIM_TILES -------------------------------------- //
        ...
        if (fread(&data_out.anim_tiles, sizeof(st_anim_map_tile)*FS_ANIM_TILES_MAX, 1, fp) == 0) {
            std::cout << ">>file_io::read_game - Error reading data from game file '" << filename << "'." << std::endl;
            exit(-1);
        }
        fclose(fp);
    ...
    int file_io::read_stage_boss_id(int stage_n, file_stage &stages_data_out)
    {
    ...
    void file_io::save_scene_sequence(std::vector<file_scene_sequence> &scene_sequence)
    {
        ...
        for (size_t i = 0; i < scene_sequence.size(); i++) {
